...
public class ScrollMechanic : MonoBehaviour, IDropHandler, IDragHandler, IBeginDragHandler, IPointerExitHandler, IPointerEnterHandler {
    ...
    public string GetCurrentName() {
        return (contentTarget.GetChild(currentCenter).GetComponentInChildren<TextMeshProUGUI>().text);
    }
     public Color GetCurrentTextColor() {
        return (contentTarget.GetChild(currentCenter).GetComponentInChildren<TextMeshProUGUI>().color);
    }
    public void RecolorText(Color color) {
       foreach(Transform child in contentTarget.transform)
       {
            child.GetComponentInChildren<TextMeshProUGUI>().color = color;
       }
    }
    public void EditPartTextWheel(string partName,Color color) {
        TextMeshProUGUI temp1;
       foreach(Transform child in contentTarget.transform)
       {
            temp1 = child.GetComponentInChildren<TextMeshProUGUI>();
            if(partName == temp1.text)
            {
                temp1.color = color;
            }
       }
    ...
    private void Update() {
        ...
        if (isInitialized) {
            ...
            if(GetCurrentName() != lastStringValue)
            {
                lastStringValue = GetCurrentName();
                FindObjectOfType<BattleHandler>().SetPartTarget(GetCurrentName());
            }
